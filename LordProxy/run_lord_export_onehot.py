...
def run_cross_validate(file_path, target_column, base_dir, cv_folds=10):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(X, y), start=1):
        output_dir = os.path.join(base_dir, f'fold{fold_idx:02d}')
        ...
        model = lord.LocalRuleClassifier_OneHotExporter()
        ...
        model.export_onehot(output_dir, y_test.tolist())
        utilities.store_data_split(X_train, y_train, os.path.join(output_dir, f"train_{fold_idx:02d}.csv"))
        utilities.store_data_split(X_test, y_test, os.path.join(output_dir, f"test_{fold_idx:02d}.csv"))
...
######################
lord.start()
# file_paths = glob.glob("data/*.csv", recursive=False)
file_paths = ["data/005.csv"]
for file_path in file_paths:
    filename = os.path.basename(file_path)
    file_name_no_ext = os.path.splitext(filename)[0]
    base_dir = os.path.join("outputs/onehot", file_name_no_ext)
